...
- `supabase/migrations/202609150001_ethone_shared_spaces_phase2.sql` — `ethone_space_events` et `ethone_space_notes`, copie mécanique exacte des 4 policies RLS de `ethone_space_tasks` (owner OU membre actif, joint via `ethone_shared_space_members`) ; colonnes `discord_guild_id`/`discord_channel_id`/`discord_linked_at` nullables sur `ethone_shared_spaces` (aucun changement RLS nécessaire, la policy propriétaire couvre déjà toute colonne). **Migration à exécuter manuellement dans le SQL Editor de Supabase.**
- `lib/hooks/useSpaceEvents.ts`, `lib/hooks/useSpaceNotes.ts` — clones quasi verbatim de `useSpaceTasks.ts` (même tracking `currentUserId`, même canal realtime, même CRUD optimiste). `components/spaces/SpaceDetailClient.tsx` : deux nouvelles cartes "Calendrier partagé"/"Notes partagées", même style d'interaction que les tâches.
- Lien Discord : `worker/src/routes/shared-spaces.js` — nouvelle branche `PATCH` sur `/api/shared-spaces`, vérification défensive du droit d'admin sur le serveur visé en relisant le blob déjà stocké par le flux OAuth Discord existant (`ethone_user_data`, aucun nouvel appel Discord depuis le Worker). `SpaceDetailClient.tsx` : nouvelle carte "Lien Discord" (propriétaire uniquement), sélecteur de serveur (réutilise `useDiscordOAuth()`) puis de salon (nouvelle route bot `GET /api/guilds/:guildId/shared-space/channels`, même convention cookie-JWT que les autres pages Discord du dashboard).
- Notifications d'activité : le Worker et le bot Discord ne se parlaient jamais avant cette passe — nouveau relais `POST /api/shared-spaces/notify` (Worker, best-effort, ne propage jamais d'erreur) → `POST /api/internal/shared-spaces/notify` (bot, service-à-service, gardé par un nouveau secret partagé `SHARED_SPACES_BOT_KEY`, comparaison `crypto.timingSafeEqual`). Livraison via un webhook dédié `"ETHONE Espaces"` (même pattern find-or-create-then-fallback que `DiscordLogService`, cache et nom séparés pour ne pas se disputer le webhook avec les logs d'audit). Déclenché sur création de tâche/événement/note et sur complétion de tâche uniquement (pas sur les autres modifications, pour limiter le bruit).
- **Configuration requise en plus de la migration** : un nouveau secret `SHARED_SPACES_BOT_KEY` à définir aux deux bouts (`wrangler secret put SHARED_SPACES_BOT_KEY` côté Worker, `SHARED_SPACES_BOT_KEY=...` dans le `.env` du bot + `pm2 restart ethone-bot`), et une variable `DISCORD_BOT_ORIGIN` (URL publique du bot) dans `worker/wrangler.jsonc` — valeurs et commandes exactes données séparément, je ne les manipule pas moi-même.
- Explicitement pas fait cette passe : synchronisation des membres du serveur Discord vers les membres de l'espace (le lien ne sert qu'aux notifications), édition riche des notes (texte simple).
- Tests : `worker/test/shared-spaces-discord-link.test.mjs` (10 cas — bits de permission, guild absente du blob, non-propriétaire → 404, unlink sans vérification), `worker/test/shared-spaces-notify.test.mjs` (8 cas — no-op si non lié, relais correct, panne bot/5xx → jamais d'erreur propagée), `discord-bot/test_shared_spaces_v1.ts` (22 assertions — sélecteur de salons, validation du payload interne, middleware de clé, logique webhook find-or-create-fallback), `lib/hooks/useSpaceEvents.test.ts`/`useSpaceNotes.test.ts` (clones de `useSpaceTasks.test.ts`).
- Validation : `worker` 249/249 tests ✓, `ethone-next` `tsc` ✓, `npm run build` ✓, `lint` 0 erreur, `test:unit` 101/101 ✓, `discord-bot` `npm run node:build` ✓ + `test_shared_spaces_v1.ts` 22/22 + `test_giveaways_v1.ts` 24/24 + `test_music_v1.ts` 31/31 (pas de régression croisée), `audit-security` PASS (1974 fichiers). Non vérifiable ici (comme d'habitude) : RLS réelle, fraîcheur du blob de permissions Discord stocké, accessibilité HTTPS réelle du bot depuis un Worker Cloudflare, création/envoi réel d'un webhook dans un vrai salon, le redéploiement PM2 lui-même.
...
- **Root cause (avant ce correctif)** : les JWT sont sans état — vérifier la signature prouve seulement que Supabase a émis le jeton, pas que la session qu'il représente est toujours censée être valide. `ethone_devices` sert déjà de table de sessions applicative (une ligne par `session_id`, voir `device-service.js`), mais `middleware/auth.js` ne la consultait jamais : une ligne « révoquée » restait pleinement fonctionnelle jusqu'à l'expiration naturelle du jeton (jusqu'à 30 jours sur le parcours OTP). `/api/signout` était un no-op de 9 lignes ne révoquant rien côté serveur, et `getOrCreateDevice`/`signServiceToken` recevaient systématiquement `sessionId: null` sur le parcours OTP — créant une nouvelle ligne d'appareil à chaque requête et rendant tout jeton émis par le Worker non révocable par construction.
- **Correctif — le mécanisme d'application réel** : `middleware/auth.js`'s `authenticateRequest()` vérifie désormais, après la validation du JWT, que le `session_id` de la revendication n'est pas marqué révoqué dans `ethone_devices` — sinon 401 `SESSION_REVOKED` immédiat. Résultats mis en cache brièvement en mémoire par isolate (même schéma que le cache JWKS existant), avec invalidation ciblée immédiate dès qu'une révocation a lieu (`invalidateSessionRevocationCache`, appelée par `revokeDevice()`) — pas d'attente de TTL pour l'isolate qui a traité la révocation. Comme Supabase conserve le même `session_id` à travers un rafraîchissement de jeton au sein d'une même connexion, ceci bloque aussi les jetons rafraîchis, pas seulement l'original.
- **Correctif de propagation** : `security-identity.js`'s parcours de vérification OTP génère désormais un vrai `crypto.randomUUID()` comme `session_id`, partagé entre la ligne d'appareil créée et le jeton émis — au lieu de `null` des deux côtés.
- **Nouvelles routes** : `POST /api/auth/device/revoke-others` (déconnecter tous les autres appareils). `POST /api/auth/device/revoke` exige désormais un flag `confirmCurrent: true` explicite pour révoquer sa propre session active (409 `CONFIRMATION_REQUIRED` sinon). `POST /api/signout` révoque réellement la session en cours et journalise un événement `signed_out`, au lieu de renvoyer un en-tête `Clear-Site-Data` inerte (scopé à l'origine du Worker, pas à `ethone.dev`).
- **Correctif du limiteur anti-force-brute** : `applyAuthRateLimit()` ne vérifiait jamais `.success` sur le chemin avec binding Cloudflare présent (donc systématiquement en production) — corrigé. Appliqué en plus aux nouvelles routes de session et à `passkey.authenticate` (identifiant : l'ID d'identifiant WebAuthn, avant authentification).
- **Migration Supabase** (`202609090002_session_revocation_hardening.sql`, à appliquer manuellement — voir message de suivi) : suppression de la contrainte `ethone_devices_not_revoked_unless_trusted`, qui interdisait littéralement la combinaison `revoked_at` + `trusted=false` que `revokeDevice()` écrit à chaque appel — cette contrainte faisait donc échouer toute révocation en base depuis la création de la table. Accès direct en écriture (`insert`/`update`/`delete`) retiré côté client sur `ethone_devices` et `insert` retiré sur `ethone_security_events` — un utilisateur pouvait jusqu'ici s'auto-remettre en confiance ou forger des entrées dans son propre journal de sécurité en contournant entièrement le Worker.
- Nouveaux tests : `worker/test/session-revocation.test.mjs` — révocation bloque la requête suivante (pas seulement le changement en base), confirmation requise pour l'auto-révocation, `revoke-others` épargne la session appelante, `/api/signout` révoque réellement, et le chemin 429 du limiteur anti-force-brute se déclenche désormais réellement.
- Validation : suite `worker` complète — **163/163** (5 nouveaux tests de révocation + 2 de la Phase 0).
- **Important — action manuelle requise** : tant que la migration `202609090002` n'est pas appliquée en base de production, `revokeDevice()` échouera avec une erreur de contrainte CHECK (comportement identique à avant ce correctif). Toute la logique applicative est prête et testée ; il manque uniquement le changement de schéma côté Supabase.

## v1.20.69 — 2026-09-09

**Correctif critique de sécurité : suppression massive non authentifiée (Phase 0 — Session & Account Security 2.0)**

Premier lot d'un chantier de sécurité complet des comptes/sessions (audit demandé par l'utilisateur : sessions actives, révocation réelle, logout-all-devices, token security, IDOR/BOLA, isolation cache/realtime, RLS, rate limiting, CSRF/CORS, Security Center, tests unitaires/intégration/E2E). Trois passes de recherche parallèles (auth frontend, architecture Worker/DB, RLS/cache/realtime) ont cartographié l'existant en profondeur avant tout code — plan complet consigné, exécution phasée. Cette version ship uniquement la Phase 0 : la faille live la plus grave trouvée, corrigée seule et en premier avant toute autre chose.

- **Root cause** : `worker/src/routes/connections.js`'s `connectionsDisconnectRoute` était monté `public: true` dans `router.js` et acceptait un flag `purgeAll` du corps de requête transmis tel quel à `disconnectProvider()` (`worker/src/services/connections-client.js`). Pour `provider: "discord"`, ce flag déclenchait 3 `DELETE` non filtrés par utilisateur sur `user_oauth_tokens`, `user_provider_credentials` et `user_data` — supprimant les identifiants Discord de **tous** les utilisateurs de la base, sans authentification. Le seul appelant historique (`ethone-next/lib/discord-migration.ts`, un script de migration ponctuelle du 4 septembre déjà documenté comme "ne s'exécute plus" dans un commentaire d'`IntegrationsSettings.tsx`) n'avait plus aucun point d'appel réel dans le code — confirmé par recherche exhaustive avant suppression.
- **Correctif** : le paramètre `purgeAll` et toute la branche de suppression globale sont supprimés de `disconnectProvider()`. La route `connections.disconnect` n'est plus publique (`public: true` retiré de `router.js`) et lève désormais explicitement `AUTH_REQUIRED` (401) si `auth.userId` est absent, au lieu de laisser `disconnectProvider` échouer silencieusement sur un `userId` manquant. Le fichier `lib/discord-migration.ts` (mort, seul appelant du flag) est supprimé.
- **Correctif associé** : trois routes de contrôle globales du bot Discord (`/api/bot` — inclut `POST /restart`, `/update` — `/api/bot/presence` — inclut `POST /identity/username`, `/identity/avatar`, `/maintenance` — et `/api/resilience` — `GET /abuse-logs`) étaient montées dans `discord-bot/src/server/index.ts` sans aucun middleware d'authentification, contrairement à leurs équivalents scopés par serveur (`/api/guilds/:guildId/bot`, etc.) qui utilisent déjà `authMiddleware` + `createGuildAuthMiddleware`. Nouveau middleware `requireBotOwner` (`discord-bot/src/server/middleware/auth.ts`) ajouté aux trois routes globales, après `authMiddleware` — nécessite désormais une session valide **et** l'identité du propriétaire du bot (`config.botOwnerId`), pas seulement une connexion Discord quelconque.
- Nouveau test de non-régression : `worker/test/connections.test.mjs` — confirme le 401 sans authentification, et que chaque `DELETE` émis reste strictement scopé à `owner_id=eq.<userId appelant>` même en repassant un `purgeAll: true` dans le corps (le champ est désormais silencieusement ignoré).
- Validation : `worker` — suite complète (158/158, dont les 2 nouveaux tests) ; `discord-bot` — `tsc --noEmit` (0 erreur), `npm run node:build`, `test_full_sync_qa.ts` (42/42), `test_bot_control_v2.ts` (40/40, harnais de test isolé non affecté par ce changement) ; `ethone-next` — `tsc --noEmit` (0 erreur) après suppression de `discord-migration.ts`.
- **Reste à faire (Phases 1 à 5 du plan)** : révocation de session réellement appliquée par le middleware Worker, isolation cache/realtime au changement de compte, correctifs TOTP/passkeys/CORS, refonte du Security Center (`/settings/security`), et suite de tests IDOR/BOLA + E2E complète. Plan détaillé conservé pour la suite du chantier.
...
- **Root cause du bug d'accent bloqué** : `VolumeSlider.tsx`, `MediaProgress.tsx`, `ProfileDropdown.tsx`, `HeroBriefingCard.tsx`, `FocusTimerRing.tsx`, `CalendarGrid.tsx`, `CalendarInvoicesPage.tsx`, `NotesPage.tsx` et `ProductivityCards.tsx` lisent tous une variable CSS `--accent-color` (parfois via `var(--accent-color, var(--accent, #10b981))`, parfois directement `var(--accent-color, #10b981)`) — mais `lib/theme-engine.ts`'s `applyTheme()`/`applyAccent()` ne l'a **jamais définie**, seulement `--accent`/`--accent-primary`. Comme `--accent-color` n'existe nulle part dans la cascade CSS, elle retombait systématiquement sur son fallback littéral (`#10b981`, un vert émeraude codé en dur), indépendamment du thème/accent réellement actif — d'où des éléments qui restent bloqués en vert après un changement de thème pendant que le reste de l'interface suit correctement le nouvel accent.
- **Correctif** : `applyTheme()` et `applyAccent()` (`lib/theme-engine.ts`) définissent désormais aussi `--accent-color` à chaque changement de thème/accent, en plus de `--accent`/`--accent-primary` déjà à jour.
- **Correctif associé** : `components/UserProfileDropdown.tsx`'s popover de profil avait un fond `bg-[#0a0b0e]` codé en dur (commentaire explicite « 100% FULL OPAQUE SOLID DARK »), ne suivant jamais le thème actif — remplacé par `bg-[var(--panel-bg)]/95` + `backdrop-blur-2xl`, la même convention que tous les autres popovers de l'app (ex. `LanguageSwitcher.tsx`), ce qui corrige aussi l'incohérence visuelle rapportée entre les différents menus déroulants du dashboard.
- **Refonte visuelle du dashboard** (agent dédié) : `DashboardOverview.tsx` (cartes de priorité de l'accueil fusionnées avec vraie hiérarchie visuelle par icône colorée), `BrainBriefingPanel.tsx` (mise en avant des tuiles à valeur non nulle), `ActivityHub.tsx` (3 occurrences de `bg-zinc-950/80` codé en dur remplacées par le token `v8-panel`, un vrai bug de thème sur `/activity`), `IntegrationsSettings.tsx` (champ de recherche unifié avec le composant `Input` du design system), `components/ui/EmptyState.tsx` repensé. Journal détaillé dans le nouveau fichier `ethone-next/DESIGN_REWORK_NOTES.md`.
- **Refonte des sons & ambiance sonore** (agent dédié) : `lib/sound.tsx` — le mixeur d'ambiances multi-couches (`SoundscapeMixer.tsx`, `FocusSoundscapeMixer.tsx`) n'était en réalité PAS multi-couches : activer un 2e son remplaçait silencieusement le premier ; corrigé avec un vrai système de couches indépendantes (`ambientLayers`/`playAmbientLayer`/`stopAmbientLayer`/`setAmbientLayerVolume`) sur un bus de gain partagé. Le réglage `uiSoundFeedback` (censé couper les sons de clic en mode Focus/Zen/Nuit) n'était jamais vérifié par `play()`/`playAction()` — corrigé. Ambiance sonore ne reprenant pas après rechargement + premier clic (politique autoplay du navigateur) — corrigé. Son de clic adouci (attaque 10ms → 15ms), transitions d'ambiance plus douces (fade-in 1.2s→1.4s, fade-out 0.4s→0.6s).
- **Refonte des embeds Discord** (agent dédié) : `/automod status` affichait tous les détecteurs en 🟢 même quand la protection est désactivée dans son ensemble — nouvel indicateur ⚫ « configuré mais pas appliqué » + bannière d'avertissement. `modLogger.ts` (logs de modération) et `sanctionService.ts` (DM de sanction) étaient encore 100% en français codé en dur malgré la traduction complète du reste du bot (v1.20.60) — threadés via i18n (fr/en/es/de), ~70 nouvelles clés ajoutées et vérifiées identiques dans les 4 langues (596 clés chacune). `discordMusicPanel.ts` avait des clés i18n déjà traduites mais jamais branchées — corrigé.
- Validation : `ethone-next` — `tsc --noEmit` (0 erreur), `npm run build`, `npm run lint` (0 erreur, 1160 warnings pré-existants), `npm run test:unit` (13/13, 61/61) ; `discord-bot` — `tsc --noEmit` (0 erreur), `npm run node:build`, `test_full_sync_qa.ts` (42/42).
...
- **Correctif critique — connexion Discord qui s'auto-révoque** : `lib/discord-migration.ts`'s `forceDisconnectDiscordAll()` (une migration ponctuelle datée du 4 sept., destinée à forcer une reconnexion après le correctif de sécurité OAuth) était câblée sur DEUX points de montage globaux (`SettingsProvider.tsx`, à chaque démarrage de l'app ; `IntegrationsSettings.tsx`, à chaque montage de la page Connexions). Pour tout navigateur n'ayant pas encore le flag "déjà exécuté" en localStorage — exactement la situation juste après qu'un utilisateur se reconnecte à Discord pour la première fois — cette migration supprimait la ligne Supabase (`ethone_user_data`, `kind=discord`) que l'échange OAuth venait tout juste d'écrire, et effaçait `ethone:connected:discord` en local, provoquant un refetch qui rapportait "déconnecté" sur la page Connexions ET sur `/discord/page.tsx` (source réelle de la liste de serveurs "Vos Serveurs (Gérables)", pas `BotControlClient.tsx`). Corrigé en retirant les deux appels à `forceDisconnectDiscordAll` (le flag ayant déjà rempli son rôle pour tout utilisateur existant).
- **Correctif — Spotify qui casse après refresh/reload** : deux régressions liées au passage de la persistance des jetons Spotify côté serveur (v1.20.52). `OAuthHandler.tsx` tentait encore d'extraire `access_token`/`refresh_token` d'une réponse d'échange qui ne les contient plus, laissant d'anciens jetons invalides traîner en `localStorage` indéfiniment ; `IntegrationsSettings.tsx`'s `handleDisconnect` ne nettoyait pas les anciennes clés non préfixées (`spotify_access_token`/`spotify_refresh_token`). `useNowPlaying.ts` retentait alors un rafraîchissement voué à l'échec à chaque poll avec `Authorization: Bearer null`, avant de retomber sur la route Worker fiable — d'où le symptôme « marche juste après reconnexion, casse au refresh ». Les trois fichiers nettoient maintenant systématiquement les jetons obsolètes.
- **Pages légales publiques** : ajout de `/terms` (Conditions d'utilisation) et `/privacy` (Politique de confidentialité), requises par Discord pour la vérification de l'application ETHONE Bot. Ajoutées à `BootProvider.tsx`'s `PUBLIC_ROUTES` pour un rendu instantané sans dépendre de la résolution de session Supabase (essentiel pour que le robot de vérification de Discord puisse les charger sans authentification).
- **Bot Discord — présence enrichie** : `BotActivity.state` (deuxième ligne de la présence, avec variables dynamiques) est désormais réellement transmis à la Gateway via `presenceService.ts` (le champ existait dans le type mais n'était jamais lu) ; exposé via une nouvelle option `ligne2` sur `/status`. Note : `details`/`assets`/`party`/`buttons` (Rich Presence complète avec image et bouton "Join") restent hors de portée d'un compte bot — ce n'est pas supporté par l'API `setPresence()` de discord.js, uniquement par un vrai client de jeu connecté en local via le Game SDK/RPC.
- Validation : `tsc --noEmit` (0 erreur), `npm run node:build` côté `discord-bot` ; `tsc --noEmit`, `npm run build`, `eslint` (0 nouvelle erreur), `npm run test:unit` (13/13 suites, 61/61 tests) côté `ethone-next`.

## v1.20.54 — 2026-09-08

**Mode "God" du Bot Owner, panneau `/settings` repensé, correctif "Unknown interaction" & fin du look "démo IA"**

- **Bot Owner "God Mode"** : immunité totale contre toute sanction (warn/ban/kick/timeout), sur tous les serveurs. Appliqué via `config.botOwnerId` dans `modules/moderation/permissions/hierarchy.ts` (`checkHierarchy()`, couvre warn/ban/kick/timeout/untimeout/nickname), `modules/automod/services/actionEngine.ts` (`executeActions()`), `modules/antiRaid/services/raidActionService.ts` (`executeMemberAction()`) et `raidDetectionService.ts` (`handleMemberJoin()`/`handleMessage()`), ainsi que 4 endroits additionnels trouvés en audit : `modules/moderation/services/sanctionService.ts` (`executeSanction`, utilisé par l'API dashboard), un second moteur historique `modules/security/services/antiNukeService.ts` + `antiRaidService.ts` (distinct du moteur `modules/antiRaid` actuel), et `modules/server/services/serverMemberService.ts` (actions déclenchées depuis le dashboard).
- **Correctif "Unknown interaction" (10062)** : `/warn` (et la même classe de bug sur `/ban`, `/kick`, `/timeout`, `/untimeout`, `/unban`, `/lock`, `/unlock`, `/nickname`, `/slowmode`, `/warnings`) répondait après des opérations lentes (fetch membre, DM, log de modération), dépassant la fenêtre de 3s de Discord — la réponse échouait silencieusement, poussant les modérateurs à relancer la commande et à avertir la cible deux fois. Corrigé par un `ctx.deferReply()` immédiat en tête de chaque commande, avant toute opération lente ; le même correctif a ensuite été étendu à d'autres modules (musique, tickets, formulaires, sondages, giveaways, suggestions, anti-raid, vocal, événements, accueil) après audit.
- **Panneau `/settings` repensé** (`commands/admin/settings.ts`, `handlers/settingsInteractionHandler.ts`) : les catégories Langue, Thème, Personnalité et Confidentialité affichent désormais un embed dédié (valeur actuelle expliquée + choix visibles) avec des boutons de sélection directe (`settings_set_personality_*`, `settings_set_theme_*`, `settings_set_privacy_*`, `settings_set_lang_*`, plus un bouton `settings_back_overview`), au lieu de cycler silencieusement à travers les valeurs à chaque clic. Les champs à saisie libre (nom du bot, préfixe, couleurs HEX personnalisées, volume/cooldown) restent en modal.
- **Nouvelle commande `/status`** (`commands/admin/statusCommand.ts`, réservée au Bot Owner car la présence Discord est globale à la connexion Gateway, pas par serveur) : changement du statut (online/idle/dnd/invisible) et de l'activité (Playing/Streaming/Listening/Watching/Competing + texte + URL) directement depuis Discord, réutilisant `PresenceService` — déjà synchronisé en temps réel avec le panneau dashboard existant `/discord/bot/presence`.
- **Traduction (i18n)** : `utils/i18n.ts` étendu de ~26 à ~254 clés (fr/en/es/de) et branché sur les commandes de modération, de musique et le panneau `/settings`. Les modules tickets/formulaires/sondages/événements/giveaways/suggestions/niveaux/automod/anti-raid ont été convertis en embeds mais restent en français (les clés de traduction correspondantes n'ont pas encore été rédigées — chantier à poursuivre dans une prochaine session).
- **Embeds partout** : plus de 150 réponses `{ content: "..." }` en texte brut converties en embeds via `ctx.createEmbed()` / `utils/embeds.ts` à travers la quasi-totalité des modules du bot (modération, musique, tickets, formulaires, sondages, giveaways, suggestions, niveaux, automod, anti-raid, vocal, événements, IA, handlers globaux).
- **Dashboard — suite du nettoyage visuel** (`ethone-next/app/discord/**`, 18 fichiers) : suppression des dernières pastilles `animate-ping`/`animate-pulse` décoratives sur les pages d'administration Discord (modération, tickets, vocal, événements, bot, sécurité anti-raid, logs, sondages, invitations, formulaires, suggestions), remplacées par un point statique + lueur douce ; quelques emojis redondants (💡✨) remplacés par de vraies icônes lucide déjà utilisées ailleurs sur la même page.
- Validation : `tsc --noEmit` (0 erreur), `npm run node:build`, `npm run build` côté `ethone-next` ; suites QA du bot Discord `test_full_sync_qa.ts` (42/42), `test_source_of_truth_reconciliation.ts` (40/40), `test_personal_voice_v2.ts` (33/33), `test_server_management_v2.ts` (23/23) ; `scripts/audit-security.mjs`.

## v1.20.53 — 2026-09-08

**Fiabilité audio du bot musical, 155 correctifs TypeScript & refonte des Réglages**

- **Bot Musical — bug « aucun son » sur `/play`** : `discord-bot/src/modules/music/providers/musicProvider.ts` contenait un mécanisme de repli qui, si le fournisseur principal échouait, envoyait l'URL de la **page** (SoundCloud/Spotify) directement à ffmpeg comme si c'était un flux audio brut — ffmpeg recevait du HTML, produisant un silence total sans aucune erreur visible. Restreint aux vraies URLs de fichiers audio directs. `discord-bot/src/modules/music/services/guildMusicPlayer.ts` : une erreur sur `resource.playStream` n'était que loggée, laissant la lecture bloquée indéfiniment sur « PLAYING » — corrigé pour déclencher `handleTrackEnd()` (passage automatique au morceau suivant).
- **Commande `/queue`** : crash (« Une erreur interne est survenue ») causé par un appel à `musicService.getQueue()`, méthode inexistante — remplacé par `getPlayer(id, false)?.getState()` dans `musicShortcuts.ts`. Bug lié corrigé au passage : `skip()` étant `async`, l'appel sans `await` faisait que `res.success` était toujours vrai (Promise toujours truthy) — les échecs de `/skip` n'étaient jamais réellement signalés.
- **155 erreurs TypeScript pré-existantes corrigées** (`discord-bot/`, 27 fichiers, 0 restante) : helper partagé `src/server/utils/params.ts` pour le typage des paramètres Express (~100 erreurs en un seul passage) ; puis correctifs unitaires révélant de vrais bugs — `settingsInteractionHandler.ts`/`language.ts` (champ `persona` renommé en `personality` côté IA, jamais mis à jour à 4+ endroits ; presets de thème du bot désynchronisés de l'enum réel) ; `backupRepository.ts` (comparaison de statut en minuscules alors que l'enum est en majuscules — une restauration interrompue n'était jamais marquée `FAILED` au redémarrage) ; `pollRoutes.ts`/`pollRepository.ts` (endpoint de création de sondage bâti sur un ancien schéma de données, cassé à l'exécution) ; `formAutomationService.ts`/`pollAutomationService.ts` (création de ticket automatique appelée avec une signature de fonction erronée) ; `inviteTrackingService.ts` (backticks non échappés dans un embed cassant la compilation).
- **Réglages — audit fonctionnel complet** (`ethone-next/components/settings/**`) : plusieurs contrôles sans aucun effet réel sont désormais connectés à l'état réel — atténuation du son sur notification et volume par ambiance sonore (`SoundscapeMixer.tsx`, `lib/sound.tsx`), bascule clair/sombre du créateur de thème personnalisé (`ThemeStudio.tsx`), badge de synchronisation reflétant l'état réel au lieu d'un texte figé (`SettingsContent.tsx`), sauvegarde du compte email/mot de passe/pseudo réellement appelée (`SettingsFormContext.tsx`), diagnostics de performance basés sur de vraies mesures et nettoyage de cache fonctionnel (`PerformanceSettings.tsx`), et les 3 interrupteurs de la Dynamic Island (`DynamicIslandSettings.tsx`, `DynamicIslandContainer.tsx`). Le toggle 2FA (aucune implémentation MFA existante) est explicitement affiché comme « Bientôt » plutôt que de simuler un fonctionnement.
- **Interface — suppression des pastilles clignotantes façon « démo IA »** : `animate-ping`/`animate-pulse` retirés des badges de statut permanents (« Connecté », « Live », mises à jour, synchronisation) sur une quinzaine de fichiers partagés (`VersionUpdateToast.tsx`, `RichToast.tsx`, `LiveFreshness.tsx`, `ConnectionCard.tsx`, `Dock.tsx`, `StatusBar.tsx`…), remplacés par un point statique + lueur douce via `--glow-color`. Les indicateurs de chargement/activité réels (Brain qui réfléchit, lecture audio en cours, sync en cours) sont conservés tels quels.
- **Cohérence visuelle** (session précédente, non encore publiée) : ~47 couleurs codées en dur remplacées par les jetons de thème dynamiques sur une quarantaine de fichiers ; correction d'une lueur verte codée en dur (`VersionUpdateToast.tsx`) visible avec le thème Dyno Rose ; début d'harmonisation des embeds du bot Discord via `discord-bot/src/utils/embeds.ts` (~19 fichiers).
- **Bot Discord — i18n** : les messages de cooldown et d'accès refusé (admin/modérateur) sont traduits dans la langue configurée du serveur (fr/en/es/de) au lieu du français uniquement.
- Validation : `tsc --noEmit` (0 erreur), `npm run node:build`, `eslint` (0 nouvelle erreur) côté `ethone-next` ; `npm run test:unit` (13 suites/61 tests) ; suites QA du bot Discord `test_full_sync_qa.ts` (42/42), `test_source_of_truth_reconciliation.ts` (40/40), `test_backup_restore_integrity_v3.ts` (48/48), `test_bot_control_v2.ts` (46/46), `test_security_isolation.ts` (32/32) ; `scripts/audit-security.mjs` et `scripts/precommit-upload-check.mjs`.
...
- Réinitialise `search` / `filter` après une déconnexion.
- Réinitialise `search` / `filter` quand un paramètre `service` est dans l’URL.
- Gère les cas `?service` inconnu ou sans catégorie.
- Validation : `npm run build`, `npm run lint`.
...
- Séparation propre des états `clientId` et `!clientId`.
- Validation : `npm run build`, `npm run lint`.
...
- `components/Shell.tsx` : fond principal uniforme (`--background`) en supprimant le dégradé vers `--panel-bg`.
- Retrait du padding-bottom excédentaire de 8rem.
- Validation : `npm run build`, `npm run lint`.
...
- `app/activity/page.tsx` : retrait de `LiveWidgets`, conteneur dédié au Activity Hub.
- `components/ActivityHub.tsx` : nouvel en-tête en `Card` avec icône et indicateur de synchronisation.
- 5 métriques d'aperçu : Aujourd'hui, Cette semaine, Série active, Moyenne/jour, Sessions.
- Validation : `npm run build`, `npm run lint`.
...
- `components/motion/shared-layout-bg.tsx` : remplacement du morphing `layoutId` par un fade/scale léger.
- `components/motion/animated-sidebar.tsx` : fond d'onglet actif en fade simple, ressort sidebar adouci.
- Ajout de `will-change-transform` sur le panneau sidebar.
- Validation : `npm run build`, `npm run lint`.
...
- La température est désormais dans `SystemStatusPills`, avec séparateurs, plutôt que dans le groupe de droite.
- Suppression de `WeatherQuickButton` de `TopBar`.
- Évite les chevauchements avec l'horloge et les autres éléments.
- Validation : `npm run build`, `npm run lint`.
...
- La TopBar est remontée tout en haut (suppression du `mt-12` lié à la Dynamic Island).
...
- Le `BootProvider` saute l’écran de chargement quand l’URL contient `?supported=true` (retour après un don Stripe).
- Version `v1.8.16`.
...
### Corrige
- `ethone-next/app/globals.css` : `--panel-obsidian` passe à 40 % de noir et `--panel-bg` à 96 % d'opacité. `--panel-border` passe à 12 % de blanc pour mieux délimiter chaque panneau. Un dégradé blanc subtil est ajouté sur `.v8-panel` et `.v8-card` pour l'effet verre obsidien.
...
- `ethone-next/components/ui/calendar.tsx` : composant `Calendar` générique avec sélection de date, dropdowns mois/année (`captionLayout="dropdown"`) et marqueurs de factures colorés.
...
- Ajustements `h-full min-h-0` sur les pages et panneaux `CalendarInvoicesPage`, `IntegrationsSettings`, `PluginClient`, `SettingsLayout`, `WeatherPage`, `activity`, `brain`, `changelog`, `files`, `flows`, `interactions`, `macros`, `matches`, `password-recovery`, `personas`, `plugins`, `profile`, `profile-selection`, `rss`, `scratchpad`, `security`, `spaces`, `team`.
