...
function updateTopbarProfile(){
  ...
  function fillAv(el,size,r){
    ...
    if(p.avatarImg){
      ...
      img.onerror=()=>{el.style.background=p.avatarBg||'rgba(var(--primary-rgb),.15)';el.innerHTML=`<span style="font-size:${Math.round(size*.44)}px;font-weight:700;color:var(--accent)">${(p.name||'U')[0].toUpperCase()}</span>`;};
