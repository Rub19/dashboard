...
type ChampionKind = "score" | "messages" | "voice" | "active" | "rising";
const CHAMPIONS: Array<{
  ...
  {
    kind: "messages",
    title: "Le plus bavard",
    hint: "messages écrits",
    unit: (v) => `${fmt(v)} msg`,
    icon: "💬",
  },
  {
  ...
  },
  {
    ...
    hint: "messages gagnés vs période précédente",
    unit: (v) => `+${fmt(v)} msg`,
    icon: "🚀",
  },
];
...
const SORTS: Array<["score" | "messages" | "voice" | "active", string]> = [
  ["score", "Score global"],
  ["messages", "Messages"],
  ["voice", "Vocal"],
  ["active", "Jours actifs"],
...
export default function StatsMembersBoard({
  base,
  days,
  refreshKey,
  onPick,
  ...
  const [sort, setSort] = useState<"score" | "messages" | "voice" | "active">(
    "score",
  );
  ...
  const [champions, setChampions] = useState<Record<
    ChampionKind,
    Champion[]
  > | null>(null);
  ...
  useEffect(() => {
    let alive = true;
    fetch(`${base}/champions?days=${days}`, { credentials: "include" })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => alive && setChampions(j?.categories ?? null))
      .catch(() => alive && setChampions(null));
    return () => {
      alive = false;
    };
  }, [base, days, refreshKey]);
    ...
    async (offset: number) => {
      ...
      try {
        const res = await fetch(
          `${base}/leaderboard?days=${days}&sort=${sort}&q=${encodeURIComponent(query)}&limit=${PAGE}&offset=${offset}`,
          { credentials: "include" },
        );
                    ...
                    {list.map((m, i) => (
                      <li key={m.id}>
                        <button
                          type="button"
                          onClick={() => onPick(m.id)}
                          className="flex w-full cursor-pointer items-center gap-2 text-left transition hover:opacity-80"
                        >
      ...
      <section className="rounded-2xl border border-[var(--panel-border)] bg-white/[0.02] p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            >
              {totals
                ? `${fmt(totals.members)} membre(s) actif(s) · ${fmt(totals.messages)} messages · ${fmt(totals.voiceHours)} h de vocal sur ${days} jours`
              ...
              <tbody className="divide-y divide-white/5">
                {rows.map((r) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-3 tabular-nums font-bold text-amber-300">
                      {fmt(r.score)}
                    </td>
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        <span className="h-1.5 w-24 overflow-hidden rounded-full bg-white/10">
                          <span
                            ...
                            style={{
                              width: `${(r.voiceHours / maxVoice) * 100}%`,
                            }}
                          />
                        </span>
                        <span className="text-[11px] text-zinc-500">
