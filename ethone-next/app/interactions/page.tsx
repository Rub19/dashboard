...
function iconForKind(kind: string): { icon: React.ElementType; color: string; label: string } {
  switch (kind) {
    ...
    case "comment":
      return { icon: MessageCircle, color: "text-[var(--info)]", label: "Commentaire" };
    case "share":
      return { icon: Share2, color: "text-[var(--accent-primary)]", label: "Partage" };
    case "noteCreate":
      return { icon: FileText, color: "text-[var(--accent-primary)]", label: "Note créée" };
    case "noteSave":
      return { icon: FileText, color: "text-[var(--accent-primary)]", label: "Note enregistrée" };
    case "taskCreate":
      return { icon: CheckSquare, color: "text-[var(--info)]", label: "Tâche créée" };
    case "taskComplete":
      return { icon: CheckCircle2, color: "text-[var(--info)]", label: "Tâche terminée" };
    case "taskDelete":
...
export default function InteractionsPage() {
  ...
  return (
    <div className="h-full min-h-0 w-full flex flex-col overflow-hidden p-4 sm:p-6">
        ...
        {[
          {
            ...
            subColor: "text-[var(--accent-primary)]",
          },
          {
      ...
      {/* Heatmap */}
      <div className="flex w-full flex-col gap-4 rounded-2xl v8-panel p-5 shadow-xl">
