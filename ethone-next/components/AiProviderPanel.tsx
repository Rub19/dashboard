      ...
      {quota && (
        <div className="rounded-[var(--panel-radius)] border border-[var(--panel-border)] bg-[var(--panel-bg)] p-4 backdrop-blur-[var(--panel-blur)]">
          ...
          <div className="mt-2 flex items-center justify-between text-sm">
            ...
            <span className={`font-medium ${quota.exhausted ? "text-red-400" : quota.prepare ? "text-amber-400" : quota.warning ? "text-yellow-400" : "text-[var(--accent-primary)]"}`}>
              {formatPercent(quota.percent)}
            </span>
          </div>
          <div className="mt-2 h-2 w-full overflow-hidden rounded-xl bg-[var(--panel-bg)]">
            <div
              className={`h-full transition-colors duration-150 ${quota.exhausted ? "bg-red-500" : quota.prepare ? "bg-amber-500" : quota.warning ? "bg-yellow-500" : "bg-[var(--accent-primary)]"}`}
