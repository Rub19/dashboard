...
export default function BulkActionBar({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface-raised)] p-3 shadow-lg">
      ...
      <div className="flex items-center gap-2">
        {onDone && (
          <button type="button" onClick={onDone} className="flex items-center gap-1.5 rounded-xl bg-[var(--surface)] px-3 py-1.5 text-xs hover:bg-[var(--accent)]/10">
        ...
        {onDelete && (
          <button type="button" onClick={onDelete} className="flex items-center gap-1.5 rounded-xl bg-red-500/10 px-3 py-1.5 text-xs text-red-400 hover:bg-red-500/20">
            <Icon name="trash-2" className="h-3.5 w-3.5" /> {i18n("delete")}
          </button>
        )}
        {children}
        <button type="button" onClick={onClear} className="rounded-xl px-3 py-1.5 text-xs text-[var(--muted)] hover:bg-[var(--surface)]">
          {i18n("cancel")}
        </button>
      </div>
    </div>
