  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-expanded={open}
        className="flex h-9 items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface-raised)] px-2.5 text-sm font-medium text-[var(--foreground)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
      >
      ...
      {open && (
        <div
          role="listbox"
          className="absolute right-0 top-full z-50 mt-2 min-w-[11rem] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-1.5 shadow-2xl backdrop-blur-xl"
        >
          {LANGUAGES.map((lang) => {
