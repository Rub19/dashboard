...
export default function LiveWidgets() {
  const { records, nowPlaying, loading } = useLiveData();
            ...
            >
              ...
              >
                <div className="absolute inset-0 h-full" style={{ backfaceVisibility: "hidden" }}>
                  <div className={`h-full min-w-0 overflow-hidden rounded-2xl border bg-gradient-to-br p-4 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl ${gradient}`}>
                        ...
                        {(nowPlaying.progressMs !== undefined && nowPlaying.durationMs) && (
                          <div className="space-y-1">
                            <div className="h-1.5 w-full overflow-hidden rounded-full bg-[var(--border)]">
                              <div className="h-full rounded-full bg-emerald-400" style={{ width: `${Math.min(100, (nowPlaying.progressMs / nowPlaying.durationMs) * 100)}%` }} />
                            </div>
                ...
                >
                  <div className="flex h-full flex-col justify-between">
                    ...
                    <div className="space-y-1 text-sm text-[var(--foreground)]">
                      <p><span className="text-[var(--muted)]">Source :</span> {record.source}</p>
                      <p><span className="text-[var(--muted)]">Statut :</span> {record.status}</p>
                      {record.subtitle && <p><span className="text-[var(--muted)]">Détail :</span> {record.subtitle}</p>}
                      {record.meta && <p><span className="text-[var(--muted)]">Meta :</span> {record.meta}</p>}
                    </div>
