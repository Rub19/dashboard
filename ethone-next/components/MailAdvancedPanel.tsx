  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          <form onSubmit={(e) => { e.preventDefault(); onCreateAccount(); }} className="space-y-2">
            ...
            {accountForm.provider === "imap" && (
              <div className="grid gap-2 sm:grid-cols-2">
                <input type="text" value={accountForm.imap_host || ""} onChange={(e) => setAccountForm({ ...accountForm, imap_host: e.target.value })} placeholder="IMAP host" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={accountForm.imap_port || "993"} onChange={(e) => setAccountForm({ ...accountForm, imap_port: e.target.value })} placeholder="IMAP port" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={accountForm.imap_username || ""} onChange={(e) => setAccountForm({ ...accountForm, imap_username: e.target.value })} placeholder="IMAP username" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="password" value={accountForm.imap_password || ""} onChange={(e) => setAccountForm({ ...accountForm, imap_password: e.target.value })} placeholder="IMAP password" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={accountForm.smtp_host || ""} onChange={(e) => setAccountForm({ ...accountForm, smtp_host: e.target.value })} placeholder="SMTP host" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={accountForm.smtp_port || "587"} onChange={(e) => setAccountForm({ ...accountForm, smtp_port: e.target.value })} placeholder="SMTP port" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={accountForm.smtp_username || ""} onChange={(e) => setAccountForm({ ...accountForm, smtp_username: e.target.value })} placeholder="SMTP username" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="password" value={accountForm.smtp_password || ""} onChange={(e) => setAccountForm({ ...accountForm, smtp_password: e.target.value })} placeholder="SMTP password" className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
            ...
            {accountForm.provider !== "imap" && (
              <input type="text" value={accountForm.access_token || ""} onChange={(e) => setAccountForm({ ...accountForm, access_token: e.target.value })} placeholder="OAuth access token" className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
            )}
            <button type="submit" disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{editingAccount ? i18n("update") : i18n("create")}</button>
          </form>
      ...
      {tab === "pgp" && (
        <div className="space-y-4">
          <Card3D>
            ...
            <div className="space-y-2">
              <input type="email" value={pgpEmail} onChange={(e) => setPgpEmail(e.target.value)} placeholder={i18n("email")} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
              <input type="password" value={pgpPassphrase} onChange={(e) => setPgpPassphrase(e.target.value)} placeholder={i18n("passphrase")} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
              <button type="button" onClick={onGeneratePgp} disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{i18n("pgpGenerate")}</button>
              {pgpGenerated && (
                <div className="space-y-1 rounded-xl bg-[var(--surface)] p-2 text-xs text-[var(--muted)] break-all">
          ...
          <Card3D>
            ...
            <div className="space-y-2">
              <input type="email" value={pgpEmail} onChange={(e) => setPgpEmail(e.target.value)} placeholder={i18n("email")} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
              <textarea value={pgpPublic} onChange={(e) => setPgpPublic(e.target.value)} placeholder={i18n("publicKey")} className="h-24 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2 text-xs" />
              <textarea value={pgpPrivate} onChange={(e) => setPgpPrivate(e.target.value)} placeholder={i18n("privateKey")} className="h-24 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2 text-xs" />
              <button type="button" onClick={onImportPgp} disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{i18n("pgpImport")}</button>
            </div>
          </Card3D>

          <div className="space-y-2">
            {pgpKeys.map((k) => (
              <div key={k.id} className="rounded-xl border border-[var(--border)] p-2 text-sm">
                <div className="flex items-center justify-between">
          ...
          <Card3D>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {!pushEnabled && <button type="button" onClick={handleSubscribePush} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white">{i18n("subscribe")}</button>}
                {pushEnabled && <button type="button" onClick={handleUnsubscribePush} className="rounded-xl border border-[var(--border)] px-3 py-2 text-sm">{i18n("unsubscribe")}</button>}
                {pushEnabled && <button type="button" onClick={handleTestPush} className="rounded-xl border border-[var(--border)] px-3 py-2 text-sm">{i18n("pushTest")}</button>}
              </div>
      ...
      {tab === "lists" && (
        <div className="space-y-4">
          <div className="grid gap-2 sm:grid-cols-2">
            <input type="text" value={listForm.name} onChange={(e) => setListForm({ ...listForm, name: e.target.value })} placeholder={i18n("name")} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
            <input type="email" value={listForm.alias_address} onChange={(e) => setListForm({ ...listForm, alias_address: e.target.value })} placeholder={i18n("alias")} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
            <input type="text" value={listForm.description} onChange={(e) => setListForm({ ...listForm, description: e.target.value })} placeholder={i18n("description")} className="sm:col-span-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
            ...
            <button type="button" onClick={onCreateList} disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{editingList ? i18n("update") : i18n("create")}</button>
          </div>

          <div className="space-y-2">
            {lists.map((l) => (
              <div key={l.id} className={`cursor-pointer rounded-xl border border-[var(--border)] p-3 text-sm ${selectedList?.id === l.id ? "bg-[var(--accent)]/10" : ""}`} onClick={() => onSelectList(l)}>
                <div className="flex items-center justify-between">
          ...
          {selectedList && (
            <Card3D>
              ...
              <div className="flex gap-2">
                <input type="email" value={memberEmail} onChange={(e) => setMemberEmail(e.target.value)} placeholder={i18n("email")} className="flex-1 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <input type="text" value={memberName} onChange={(e) => setMemberName(e.target.value)} placeholder={i18n("name")} className="flex-1 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <button type="button" onClick={onAddMember} disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{i18n("add")}</button>
              </div>
              <div className="space-y-2">
                {members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between rounded-xl border border-[var(--border)] p-2 text-sm">
              ...
              <div className="space-y-2 border-t border-[var(--border)] pt-3">
                ...
                <input type="text" value={listMessage.subject} onChange={(e) => setListMessage({ ...listMessage, subject: e.target.value })} placeholder={i18n("subject")} className="w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm" />
                <textarea value={listMessage.body_text} onChange={(e) => setListMessage({ ...listMessage, body_text: e.target.value })} placeholder={i18n("body")} className="h-24 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] p-2 text-sm" />
                <button type="button" onClick={onSendToList} disabled={busy} className="rounded-xl bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{i18n("send")}</button>
              </div>
              </div>
            </Card3D>
          )}
