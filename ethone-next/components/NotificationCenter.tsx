        ...
        {FILTERS.map((f) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-1 items-center justify-center gap-1.5 rounded-lg py-1.5 text-xs font-bold transition-all cursor-pointer",
                filter === f.id
    ...
    >
      <PopoverTrigger>
        ...
        >
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 flex h-4 min-w-[1rem] items-center justify-center rounded-full bg-[var(--accent-primary)] px-1 text-[9px] font-bold text-[var(--accent-contrast)] shadow-md animate-pulse">
      ...
      {!isMobile && (
        <PopoverContent className="w-[420px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-[var(--panel-border)] bg-[var(--panel-bg)]/95 p-4 shadow-2xl backdrop-blur-2xl">
          {content}
        </PopoverContent>
      )}
