...
export default function OwnerShieldPanel({ isOwner }: OwnerShieldPanelProps) {
  ...
  const fetchStatus = useCallback(async (notify = false) => {
    ...
    try {
      ...
      if (json.success && json.data) {
        setAutoDefense(Boolean(json.data.autoDefenseEnabled));
  ...
  const handleToggleAutoDefense = async () => {
    ...
    try {
      const res = await fetch(`${BOT_API_URL}/api/bot/owner-shield/toggle`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-bot-owner": OWNER_DISCORD_ID,
      ...
      if (res.ok) {
        ...
        success(`Protection automatique ${newState ? "activée" : "désactivée"} !`);
      }
    } catch {
      showError("Échec de mise à jour de la protection");
    }
  };
  ...
  if (!isOwner) {
    return (
      <Card variant="default" padding="none" className="p-8 text-center space-y-3">
        ...
        <h3 className="text-base font-bold text-[var(--text-primary)]">Accès Réservé au Propriétaire Suprême</h3>
        <p className="text-xs text-[var(--text-muted)] max-w-md mx-auto">
          Ce centre de commande d'urgence et de contre-mesures est strictement réservé au fondateur du bot et du dashboard (<code className="text-rose-400">rub19.mailpro@gmail.com</code> / <code className="text-rose-400">825124006209388616</code>).
        </p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* HEADER BANNER */}
      <Card variant="default" padding="none" className="p-6 space-y-5 border-amber-500/30 bg-gradient-to-br from-amber-500/5 via-transparent to-rose-500/5">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-[var(--panel-border)]">
          <div className="space-y-1">
            ...
            <h2 className="text-lg font-black text-[var(--text-primary)] tracking-tight flex items-center gap-2">
              ...
              Bouclier Suprême & Sauvetage de l'Owner
            </h2>
            <p className="text-xs text-[var(--text-muted)] max-w-2xl">
              Protection active contre toute sanction externe (bannissement, timeout, mute vocal ou rôle prison).
              Le bot utilise sa position hiérarchique élevée pour vous auto-débannir, lever vos restrictions et vous réinviter instantanément.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className={cn(
                "h-9 px-4 rounded-xl text-xs font-bold transition-all flex items-center gap-2 cursor-pointer",
                autoDefense
                  ? "bg-emerald-600 hover:bg-emerald-500 text-white shadow-sm"
                  : "bg-zinc-800 hover:bg-zinc-700 text-zinc-300"
              )}
            >
              <span className={cn("w-2 h-2 rounded-full", autoDefense ? "bg-white animate-pulse" : "bg-zinc-500")} />
              <span>Auto-Défense : {autoDefense ? "ACTIVE" : "INACTIVE"}</span>
            </button>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card variant="widget" padding="sm" className="space-y-1">
            ...
            <p className="text-xl font-black text-amber-400">{guilds.length}</p>
          </Card>
          <Card variant="widget" padding="sm" className="space-y-1">
            <span className="text-[11px] text-[var(--text-muted)] font-medium">Sanctions Actives Détectées</span>
          ...
          <Card variant="widget" padding="sm" className="space-y-1">
            <span className="text-[11px] text-[var(--text-muted)] font-medium">Interceptions Automatiques</span>
            <p className="text-xl font-black text-indigo-400">{history.length}</p>
          </Card>
          <Card variant="widget" padding="sm" className="space-y-1">
            <span className="text-[11px] text-[var(--text-muted)] font-medium">Commande Discord</span>
            <p className="text-xs font-mono font-bold text-rose-300">/rescue ou !rescue</p>
          </Card>
        </div>
      </Card>

      {/* DIAGNOSTIC PAR SERVEUR */}
        ...
        <div className="flex items-center justify-between pb-2 border-b border-[var(--panel-border)]">
          <div>
            <h3 className="text-sm font-bold text-[var(--text-primary)] flex items-center gap-2">
              ...
              Diagnostic & État de l'Owner par Serveur ({guilds.length})
            </h3>
            <p className="text-xs text-[var(--text-muted)]">
              Contrôles d'urgence pour chaque serveur où le bot est installé
            </p>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-1">
                          ...
                          {!hasActiveSanction && st.isPresent && (
      ...
      <Card variant="default" padding="none" className="p-6 space-y-4">
        ...
        {history.length === 0 ? (
          <div className="py-8 text-center rounded-xl bg-[var(--text-primary)]/[0.02] border border-[var(--panel-border)] text-xs text-[var(--text-muted)] space-y-1">
            ...
            <p className="font-semibold text-[var(--text-primary)]">Aucune tentative malveillante récente</p>
