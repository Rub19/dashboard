  ...
  return (
    <div className="flex flex-col items-center max-w-2xl mx-auto py-2 sm:py-6 px-4 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="v8-panel w-full p-5 mb-5 relative">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-[var(--text-muted)]">{s.step}</span>
                    <span className="text-xs font-bold text-[var(--text-primary)] truncate">{s.title}</span>
                  </div>
                  <div className="text-[11px] text-[var(--text-muted)] truncate">{s.desc}</div>
                </div>

                {idx < steps.length - 1 && (
                  <div className="hidden sm:flex text-[var(--text-muted)]">
        ...
        <div className="mt-4 pt-3 border-t border-[var(--panel-border)] flex items-center justify-between text-xs text-[var(--text-muted)]">
          <span>Temps d'exécution moyen du workflow :</span>
          <span className="text-[var(--success)] font-bold">&lt; 150 ms total</span>
        </div>
      </div>
