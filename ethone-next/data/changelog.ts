const v12044_fr: ChangelogEntry = {
  version: "v1.20.44",
  date: "2026-09-04",
  title: "ETHONE Mail 2.0 — Résolution de la Création d'Adresse & Prévol CORS En-Têtes",
  items: [
    "Création d'adresse @ethone.dev : éradication complète de l'erreur bloquante « Impossible de joindre le serveur ETHONE » lors de la création d'adresse email sur /mail.",
    "Ciblage des En-têtes Tiers : fetchWorker n'injecte désormais les en-têtes d'API (Tracker, Riot, Henrik, Spotify) que pour les routes de services correspondantes, évitant tout rejet de prévol CORS sur les routes mail.",
    "Allowlist CORS Complète : ajout de tous les en-têtes client autorisés dans worker/src/middleware/cors.js (x-riot-api-key, x-henrik-api-key, x-tracker-api-key, x-spotify-token, x-ethone-theme, x-ethone-version).",
    "Onboarding Mail Résilient : prise en charge immédiate des alias déjà rattachés au compte utilisateur, prévention des soumissions multiples et messages d'erreur conviviaux.",
  ],
};

const v12044_en: ChangelogEntry = {
  version: "v1.20.44",
  date: "2026-09-04",
  title: "ETHONE Mail 2.0 — Email Address Setup & CORS Header Resolution",
  items: [
    "@ethone.dev Address Creation: resolved the blocking 'Impossible de joindre le serveur ETHONE' error when setting up an address on /mail.",
    "Targeted API Headers: fetchWorker now restricts third-party headers (Tracker, Riot, Henrik, Spotify) strictly to their dedicated service paths, preventing CORS preflight failures on mail routes.",
    "Comprehensive CORS Allowlist: added all valid client extension headers to worker/src/middleware/cors.js (x-riot-api-key, x-henrik-api-key, x-tracker-api-key, x-spotify-token, x-ethone-theme, x-ethone-version).",
    "Resilient Mail Onboarding: immediate recognition of user-owned aliases, multi-click debounce protection, and user-friendly error messages.",
  ],
};

const v12043_fr: ChangelogEntry = {
  version: "v1.20.43",
  date: "2026-09-04",
  title: "ETHONE Connexions & Discord 2.0 — Révocation et Déconnexion Forcée Globale",
  items: [
    "Déconnexion Forcée Globale de Discord : révocation automatique et purge des anciennes sessions Discord pour l'ensemble des utilisateurs afin d'assurer une migration propre vers le nouveau bot (App ID 1545139931154878464).",
    "Nettoyage Local Immédiat : suppression synchrone des clés de connexion Discord orphelines dans `localStorage` (`ethone:connected:discord`, `ethone:token:discord`, `ethone:pub:discord`, `liveLanyardUserId`) sur chaque client.",
    "Purge Distante via Cloudflare Worker & Supabase : nouvel endpoint `/api/connections/disconnect` avec option de purge globale supprimant les tokens révoqués dans `user_oauth_tokens`, `user_provider_credentials` et `user_data`.",
    "Protection Anti-Ghost Connection : mise à jour de `isConfigured` empêchant tout affichage intempestif de Discord comme connecté tant qu'une nouvelle connexion explicite n'a pas été établie avec le nouveau bot.",
  ],
};

const v12043_en: ChangelogEntry = {
  version: "v1.20.43",
  date: "2026-09-04",
  title: "ETHONE Connections & Discord 2.0 — Platform-Wide Forced Disconnect & Session Purge",
  items: [
    "Platform-Wide Discord Forced Disconnect: automatic revocation and purge of legacy Discord sessions across all users to guarantee a clean migration to the new bot (App ID 1545139931154878464).",
    "Instant Client-Side Cache Cleanup: synchronous purge of orphan Discord keys in `localStorage` (`ethone:connected:discord`, `ethone:token:discord`, `ethone:pub:discord`, `liveLanyardUserId`) on every device.",
    "Remote Purge via Cloudflare Worker & Supabase: added `/api/connections/disconnect` with global purge flag clearing revoked tokens in `user_oauth_tokens`, `user_provider_credentials`, and `user_data`.",
    "Anti-Ghost Connection Guard: updated `isConfigured` logic preventing stale ghost sessions from showing Discord as connected until a fresh link is established with the new bot.",
  ],
};

const v12042_fr: ChangelogEntry = {
  version: "v1.20.42",
  date: "2026-09-04",
  title: "ETHONE Identity & Profile Engine 2026 — Sauvegarde et Synchronisation Résiliente",
  items: [
    "Persistance Multi-Couche de l'Identité : résolution complète du bug de sauvegarde sur /profile (photo de profil, nom d'affichage, username, bio, cadres d'avatar).",
    "Sauvegarde Instantanée & Locale : écriture synchrone dans l'ensemble des clés localStorage (`ethone:identity`, `ethone_user_name`, `ethone_custom_avatar`, `ethone_user_frame`, etc.) garantissant zéro perte de données même hors-ligne.",
    "Supabase Upsert Automatique : migration des requêtes update vers `.upsert(..., { onConflict: 'user_id' })` sur la table `ethone_public_profiles` et synchronisation auth `user_metadata`.",
    "Prévisualisation Réactive en Direct : le composant `ProfileHero2026` affiche instantanément vos changements (nouveau cadre d'avatar, nom, @handle, bio) pendant la saisie et le clic.",
    "Préservation des Formulaires : élimination du clobbering asynchrone des inputs lors des rechargements d'état en arrière-plan grâce au tracking `userEditedRef`.",
    "Connexions & Bot Discord 2.0 : alignement complet de la page Connexions (/connections) avec le nouveau bot Discord (App ID 1545139931154878464) et intégration du dashboard /discord.",
  ],
};

const v12042_en: ChangelogEntry = {
  version: "v1.20.42",
  date: "2026-09-04",
  title: "ETHONE Identity & Profile Engine 2026 — Resilient Profile Persistence & Live Sync",
  items: [
    "Multi-Layer Identity Persistence: definitive fix for the /profile save issue (avatar picture, display name, username, bio, and avatar frames now persist flawlessly).",
    "Instant Local Cache: immediate synchronous writes across all localStorage keys (`ethone:identity`, `ethone_user_name`, `ethone_custom_avatar`, `ethone_user_frame`, etc.) preventing any state loss.",
    "Automated Supabase Upsert: migrated update calls to `.upsert(..., { onConflict: 'user_id' })` on `ethone_public_profiles` with Supabase Auth `user_metadata` synchronization.",
    "Real-Time Reactive Preview: `ProfileHero2026` immediately mirrors your changes (avatar frame, display name, @handle, bio) in real-time as you type or pick a cosmetic.",
    "Form State Preservation: eliminated unwanted background async state clobbering via `userEditedRef` input tracking.",
    "Discord Bot 2.0 & Connections: aligned Connections page (/connections) with the new Discord Bot (App ID 1545139931154878464) and direct dashboard routing.",
  ],
};
...
CHANGELOG_BY_LANG.fr.unshift({
  ...
  items: [
    "Intégration du moteur MarkdownContent pour un rendu visuel complet des réponses de Brain (titres, puces, checklists interactives, blocs de citation et de code).",
    "Enregistrement automatique et fidèle du contenu Markdown complet des notes générées par l'IA dans l'espace Notes.",
    "Boutons de copie rapide et d'action enrichis dans le chat.",
  ],
});
...
CHANGELOG_BY_LANG.en.unshift({
  version: "v1.12.62",
  date: "2026-08-30",
  title: "Rich Markdown AI Rendering & Complete Note Saving",
  items: [
    "Integrated rich MarkdownContent renderer for Brain AI chat (headers, bullet points, checklists, quotes, and code blocks).",
    "Automatic complete Markdown body persistence into Notes when generated by AI.",
    "Quick copy and direct action buttons in Brain chat.",
  ],
});
...
CHANGELOG_BY_LANG.es.unshift({
  version: "v1.12.62",
  date: "2026-08-30",
  title: "Renderizado Markdown y Guardado de Notas",
  items: [
    "Renderizado visual Markdown completo para el chat de IA y guardado estructurado en Notas.",
  ],
});
...
CHANGELOG_BY_LANG.de.unshift({
  version: "v1.12.62",
  date: "2026-08-30",
  title: "Reichhaltiges Markdown-Rendering & Notizspeicherung",
  items: [
    "Vollständiges visuelles Markdown-Rendering für Brain-Chat und automatische Speicherung in Notizen.",
  ],
});
