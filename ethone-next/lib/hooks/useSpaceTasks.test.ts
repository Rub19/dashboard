...
import { useSpaceTasks } from "./useSpaceTasks";
...
describe("useSpaceTasks", () => {
  ...
  it("loads tasks scoped to the given space_id", async () => {
    const { result } = renderHook(() => useSpaceTasks("space-1"));
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(mockedSupabase.from).toHaveBeenCalledWith("ethone_space_tasks");
  ...
  it("subscribes to a realtime channel filtered by space_id once the user is known", async () => {
    ...
    renderHook(() => useSpaceTasks("space-1"));
  ...
  it("tears down the old channel and opens a new one when spaceId changes", async () => {
    ...
    const { rerender } = renderHook(({ spaceId }: { spaceId: string }) => useSpaceTasks(spaceId), {
