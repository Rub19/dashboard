...
export const useSyncStore = create<SyncStore>((set) => ({
  ...
  setStatus: (source, status) =>
    set((state) => {
      ...
      // If a source stays "syncing" for too long, automatically recover back to "idle".
      // This prevents UI spinners from turning in loops indefinitely.
      if (status === "syncing") {
        const timeout = setTimeout(() => {
          set((s) => {
            if (s.sources[source] !== "syncing") return s;
            const updated: Record<string, SyncState> = { ...s.sources, [source]: "idle" };
