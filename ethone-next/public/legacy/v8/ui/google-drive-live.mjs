...
function dayMonthFormatter() {
  return new Intl.DateTimeFormat(currentLocale(), { day: "numeric", month: "short" });
}

function formatModified(value) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return `${translateSource("Modifié le")} ${dayMonthFormatter().format(date)}`;
}
...
export function googleDriveLiveCard(presence = {}, options = {}) {
  ...
  const front = element("div", { className: `v8-google-drive-live v8-google-drive-live--${variant} v8-surface v8-live-card-front` }, [
    ...
    element("div", { className: "v8-google-drive-live__body" }, [
      element("div", { className: "v8-google-drive-live__meta" }, [brandIcon("google-drive", "file", "v8-live-brand-mark"), element("small", { text: translateSource("Dernier fichier modifié") })]),
  ...
  const back = element("div", { className: "v8-live-card-back v8-google-drive-live-back" }, [
    ...
    element("footer", { className: "v8-flip-back-footer" }, [element("small", { text: translateSource("Dernier fichier modifié") })])
  ]);
