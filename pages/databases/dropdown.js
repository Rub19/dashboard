/* ETHONE Database Builder — shared floating dropdown panel (duplicated from pages/valorant-accounts/dropdown.js, db-prefixed for independence). */
(function(){
  "use strict";
  ...
  function buildList(query){
    ...
    catOrder.forEach(function(cat){
      if(cat)html+='<div class="db-dd-cat">'+esc(cat)+"</div>";
      byCat[cat].forEach(function(it){
        var sel=isSelected(it.value);
        html+='<button type="button" class="db-dd-opt'+(sel?" selected":"")+'" data-value="'+esc(it.value)+'" role="option" aria-selected="'+sel+'">'+
          (it.color?'<span class="db-dd-dot" style="background:'+esc(it.color)+'"></span>':"")+
          (it.icon?'<span class="db-dd-icon">'+it.icon+"</span>":"")+
          '<span class="db-dd-label">'+esc(it.label)+"</span>"+
          (state.opts.multi?'<span class="db-dd-check">'+(sel?CHECK_SVG:"")+"</span>":(sel?'<span class="db-dd-check">'+CHECK_SVG+"</span>":""))+
        "</button>";
      });
    });
    if(!filtered.length){
      html+='<div class="db-dd-empty">Aucun résultat</div>';
    }
    if(state.opts.allowCreate&&query&&query.trim()){
      var exists=items.some(function(it){return normalize(it.label)===normalize(query);});
      if(!exists){
        html+='<button type="button" class="db-dd-opt db-dd-create" data-create="1"><span class="db-dd-icon">+</span><span class="db-dd-label">Créer « '+esc(query.trim())+" »</span></button>";
      }
    }
    return html;
  }
  ...
  function renderPanel(){
    var p=ensurePanel();
    var opts=state.opts;
    var searchHTML=opts.searchable===false?"":'<div class="db-dd-search-wrap"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="13" height="13"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" class="db-dd-search" id="db-dd-search-input" placeholder="'+esc(opts.placeholder||"Rechercher…")+'" autocomplete="off"></div>';
    p.innerHTML=(opts.title?'<div class="db-dd-title">'+esc(opts.title)+"</div>":"")+searchHTML+'<div class="db-dd-list" id="db-dd-list"></div>';
    var listEl=p.querySelector("#db-dd-list");
    listEl.innerHTML=buildList("");
    var searchInput=p.querySelector("#db-dd-search-input");
    if(searchInput){
      searchInput.addEventListener("input",function(){listEl.innerHTML=buildList(searchInput.value);});
      searchInput.addEventListener("keydown",function(e){
        if(e.key==="Escape"){close();}
        if(e.key==="Enter"){
          var createBtn=listEl.querySelector(".db-dd-create");
          if(createBtn)createBtn.click();
          else{
            var first=listEl.querySelector(".db-dd-opt");
    ...
    listEl.addEventListener("click",function(e){
      var btn=e.target.closest(".db-dd-opt");
      if(!btn)return;
  ...
  function close(){
    ...
    if(activeTrigger)activeTrigger.classList.remove("db-dd-trigger-active");
    activeTrigger=null;
  ...
  function open(anchor, opts){
    ...
    anchor.classList.add("db-dd-trigger-active");
    state={opts:opts, selected: opts.multi?(opts.selected||[]).slice():opts.selected};
    renderPanel();
    position(anchor);
    var searchInput=panel.querySelector("#db-dd-search-input");
    if(searchInput)setTimeout(function(){searchInput.focus();},10);
  }

  document.addEventListener("mousedown",function(e){
    if(panel&&panel.classList.contains("open")&&!e.target.closest(".db-dd-panel")&&!e.target.closest(".db-dd-trigger-active")){
      close();
    }
  });
  ...
  window.dbOpenDropdown=open;
  window.dbCloseDropdown=close;
})();
