...
test("interaction feedback preserves semantic states and versions every stylesheet", () => {
  ...
  for (const name of ["tokens", "base", "components", "entry", "presence"]) {
    assert.match(html, new RegExp(`v8/styles/${name}\\.css\\?v=experience-v180`));
    assert.match(worker, new RegExp(`v8/styles/${name}\\.css\\?v=experience-v180`));
  }
  assert.match(worker, /v8\/styles\/activity\.css\?v=experience-v180/);
