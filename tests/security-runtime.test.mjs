...
test("browser CSP is exact-origin and blocks script attributes", () => {
  ...
  assert.equal(policy.match(/img-src\s+([^;]+)/)?.[1], "'self' data: blob: https://i.scdn.co https://cdn.discordapp.com https://textures.minecraft.net https://avatars.githubusercontent.com https://avatars.steamstatic.com https://avatars.akamaihd.net https://media.valorant-api.com https://bvgifyzhpzkbrwdjrqsg.supabase.co");
...
test("service worker waits for explicit activation and only removes ETHONE caches", () => {
  ...
  assert.match(worker, /2026-07-28-experience-v134/);
});
