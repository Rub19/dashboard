...
export function mountApplication(root, options = {}) {
  ...
  const releaseSpotify = spotifyLive.subscribe?.((playback) => {
    ...
    if (typeof document !== "undefined") {
      document.documentElement.dataset.v8AuraPulse = playback.playing ? "true" : "false";
    }
  }, { immediate: false }) || (() => {});
  ...
  function applyPageTransition(update) {
    if (typeof document === "undefined" || !document.startViewTransition) return Promise.resolve(update());
  ...
  function mountLazyRoute(route, focus, requestId) {
    ...
    if (lazyModuleCache.has(route) && (!needsBrain || brainRuntime)) {
      ...
      applyPageTransition(() => {
        lifecycle.unmount();
        lifecycle.mount(route, () => lazyPageMount(route, module, brain));
    ...
    applyPageTransition(() => {
      ...
      return Promise.all([loader(), needsBrain ? ensureBrainRuntime() : Promise.resolve(null)]);
  ...
  function mountRoute(route, focus = true, navigation = null) {
    ...
    const doMount = () => {
      lifecycle.mount(route, () => {
        ...
        if (route === "calendar") return mountCalendar(shell.stage, { repository, actions, presence, notify: (notice) => toasts.show(notice), state: store.getState(), subscribeState: store.subscribe, externalServices });
    ...
    if (document.startViewTransition) {
      document.startViewTransition(doMount);
    } else {
      doMount();
    }
  }
