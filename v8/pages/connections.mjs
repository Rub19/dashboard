...
export function mountConnections(stage, options = {}) {
  ...
  function renderCatalog() {
    ...
    const visible = filterConnectionCatalog(INTEGRATIONS, { category, status, query, connections });
    grid.replaceChildren(...visible.map((integration) => connectionCard(integration, map.get(integration.id), selectedId)));
    if (!visible.length) {
      ...
      grid.append(emptyState({ iconName: "search-x", eyebrow: "Catalogue", title: "Aucune integration trouvee", description: "Essayez un autre service, statut ou categorie.", actions: [reset], className: "v8-empty-state--wide" }));
    }
    resultCount.textContent = `${visible.length} / ${INTEGRATIONS.length}`;
  ...
  function setupPanel(integration, connection, method) {
    ...
    if (method?.field) {
      const input = element("input", {
        className: "v8-input",
        attributes: { type: method.field.type === "url" ? "url" : "text", value: referenceValue, placeholder: method.field.placeholder, autocomplete: "off", spellcheck: "false", "aria-label": method.field.label },
        dataset: { connectionReference: integration.id }
      });
      input.addEventListener("input", () => draftReferences.set(integration.id, input.value), { signal: controller.signal });
      referenceField = element("label", { className: "v8-connection-reference" }, [
        element("span", {}, [element("strong", { text: method.field.label }), element("small", { text: method.availability === "local" ? "Boucle locale uniquement" : "Ne collez jamais une adresse privee ou signee" })]),
        element("div", { className: "v8-connection-reference__control" }, [input, referenceValue ? connectionAction("v8.connections.copy", integration.id, "secondary", [icon("copy")], { className: "v8-icon-button", ariaLabel: "Copier la valeur", value: referenceValue, tooltip: "Copier" }) : null])
      ]);
    }
    return element("div", { className: "v8-connection-inspector-panel", attributes: { role: "tabpanel" } }, [
  ...
  function diagnosticsPanel(integration, connection, method) {
    ...
    return element("div", { className: "v8-connection-inspector-panel", attributes: { role: "tabpanel" } }, [
      ...
      ]) : element("div", { className: "v8-connection-diagnostic-empty", attributes: { "aria-live": "polite" } }, [icon(loading ? "loader-circle" : "stethoscope"), element("strong", { text: loading ? "Diagnostic en cours" : "Aucun diagnostic execute" }), element("p", { text: loading ? "Verification de la route Worker sans appel fournisseur inutile." : "Le test ne lance aucun flux OAuth et ne revele aucune donnee sensible." })]),
  ...
  releases.push(actions.scope("v8.connections.setup.complete", (context) => {
    ...
    if (!reference.ok) {
      ...
      inspectorHost.querySelector("[data-connection-reference]")?.focus({ preventScroll: true });
