...
const THEME_OPTIONS = Object.freeze([
  ...
  Object.freeze({ id: "day", label: "Jour", icon: "sun", copy: "Clair, pour la lumiÃ¨re du jour.", swatch: Object.freeze({ canvas: "#f4f5f7", surface: "#ffffff", text: "#161a21" }) }),
  Object.freeze({ id: "auto", label: "Automatique", icon: "monitor", copy: "Suit les prÃ©fÃ©rences de votre systÃ¨me.", swatch: null })
]);
const BRAIN_PERMISSION_LABELS = Object.freeze({ notes: "Notes", tasks: "Taches", calendar: "Calendrier", connections: "Connexions", gaming: "Gaming", activity: "ActivitÃ©", files: "Fichiers", profile: "Profil", settings: "RÃ©glages" });
const BRAIN_MEMORY_LABELS = Object.freeze({ interface: "Interface", habits: "Habitudes", widgets: "Widgets", schedules: "Plannings", "task-types": "Types de taches", spaces: "Spaces", flows: "Flows", "response-style": "Style de rÃ©ponse", goals: "Objectifs" });
...
const DENSITY_LABELS = Object.freeze({ spacious: "Spacieuse", comfortable: "Confortable", compact: "Compacte", "ultra-compact": "Ultra compacte", automatic: "Automatique", custom: "PersonnalisÃ©e" });
const DENSITY_OPTIONS = Object.freeze([
  ...
  Object.freeze({ id: "ultra-compact", label: "Ultra compacte", icon: "list-collapse", copy: "DensitÃ© maximale avec focus et cibles conserves." }),
  Object.freeze({ id: "automatic", label: "Automatique", icon: "wand-sparkles", copy: "S'adapte a l'Ã©cran, au zoom et au contexte." }),
  Object.freeze({ id: "custom", label: "PersonnalisÃ©e", icon: "sliders-horizontal", copy: "Reglez chaque dimension de l'interface." })
]);
const CUSTOM_DENSITY_LABELS = Object.freeze({ fontScale: "Taille du texte", lineHeight: "Interligne", cardPadding: "Padding des cartes", sectionGap: "Espacement des blocs", controlHeight: "Hauteur des boutons", panelWidth: "Largeur des panneaux", iconSize: "Taille des icÃ´nes", rowHeight: "DensitÃ© des listes", tableRowHeight: "DensitÃ© des tableaux", widgetScale: "Taille des widgets", toolbarHeight: "Hauteur des toolbars" });
const SOUND_VOLUME_ROWS = Object.freeze([
  Object.freeze({ id: "master", icon: "volume-2", title: "Volume gÃ©nÃ©ral", description: "Limiter le niveau de tout ETHONE." }),
  Object.freeze({ id: "notifications", icon: "bell-ring", title: "Notifications", description: "Informations, succÃ¨s, alertes et mises a jour." }),
  Object.freeze({ id: "interface", icon: "mouse-pointer-2", title: "Interface", description: "FenÃªtres, commandes et interactions importantes." }),
  Object.freeze({ id: "brain", icon: "brain", title: "Brain", description: "Reflexion, rÃ©ponse et fin de traitement." }),
  Object.freeze({ id: "system", icon: "audio-lines", title: "SystÃ¨me", description: "Connexion, sauvegarde, synchronisation et Spaces." })
]);
const AURA_OPTIONS = Object.freeze([
  Object.freeze({ id: "classic", label: "Classique", icon: "sparkles", copy: "Ambiance ETHONE par dÃ©faut." }),
  Object.freeze({ id: "boreale", label: "BorÃ©ale", icon: "zap", copy: "Cyan borÃ©al et violet intense." }),
  Object.freeze({ id: "cyberpunk", label: "Cyberpunk", icon: "flame", copy: "Magenta nÃ©on & Ã©meraude vif." }),
  Object.freeze({ id: "eclipse", label: "Ã‰clipse", icon: "moon", copy: "Lueur or sur nuit stellaire." }),
  Object.freeze({ id: "emeraude", label: "Ã‰meraude", icon: "gem", copy: "Vert jade prÃ©cieux & saphir." }),
  Object.freeze({ id: "minerale", label: "MinÃ©rale", icon: "mountain", copy: "Ardoise brute et platine." })
]);
const FONT_OPTIONS = Object.freeze([
  Object.freeze({ id: "inter", label: "Inter (Moderne)", icon: "type", copy: "Sans-serif Ã©quilibrÃ© par dÃ©faut." }),
  Object.freeze({ id: "outfit", label: "Outfit (GÃ©omÃ©trique)", icon: "sparkles", copy: "Rendu dynamique et Ã©lÃ©gant." }),
  Object.freeze({ id: "mono", label: "JetBrains Mono", icon: "code", copy: "Esprit technique et cyberpunk." }),
  Object.freeze({ id: "serif", label: "Editorial Serif", icon: "book-open", copy: "Typographie littÃ©raire et premium." })
]);
const RADIUS_OPTIONS = Object.freeze([
  ...
  Object.freeze({ id: "soft", label: "Courbe iOS", icon: "smile", copy: "Courbes gÃ©nÃ©reuses (12px / 20px)." })
]);
...
function soundRange(category, value, disabled) {
  ...
  const label = category === "master" ? "Volume gÃ©nÃ©ral" : `Volume ${category}`;
...
function themeModeChoice(option, active, resolution) {
  const resolvedNote = option.id === "auto"
    ? element("small", { className: "v8-thÃ¨me-choice__resolved", text: `-> ${THEME_LABELS[resolution?.effective || "night"]}`, attributes: { hidden: resolution?.requested !== "auto" } })
    : null;
  return element("button", {
...
function densityPreview() {
  return element("div", { className: "v8-density-preview", attributes: { "aria-label": "Apercu interactif de la densitÃ©" } }, [
    ...
    element("div", { className: "v8-density-preview__workspace" }, [
      ...
      element("article", { className: "v8-density-preview__card" }, [element("small", { text: "Dashboard" }), element("strong", { text: "Votre espace en un regard" }), element("p", { text: "Cartes, listes et commandes utilisent les mÃªmes tokens." }), element("span", { className: "v8-density-preview__demo-control", attributes: { "aria-hidden": "true" } }, [icon("sparkles"), element("span", { text: "Action" })])]),
      element("div", { className: "v8-density-preview__list" }, ["PrioritÃ© principale", "Briefing Brain", "Synchronisation Supabase"].map((label, index) => element("span", {}, [icon(index === 0 ? "circle-check-big" : index === 1 ? "brain" : "cloud"), element("b", { text: label })])))
    ]),
...
export function mountSettings(stage, options = {}) {
  ...
  const densityChoices = element("div", { className: "v8-density-options", attributes: { role: "group", "aria-label": "Mode de densitÃ©" } }, DENSITY_OPTIONS.map((option) => densityModeChoice(option, state.density === option.id, option.id === "custom" ? initialDensitySettings.custom : option.id === "automatic" ? DENSITY_PRESETS.comfortable : null)));
  ...
  const themeChoices = element("div", { className: "v8-theme-options", attributes: { role: "group", "aria-label": "ThÃ¨me" } }, THEME_OPTIONS.map((option) => themeModeChoice(option, state.theme === option.id, themeResolution)));
  ...
  const brainDetailSelect = preferenceSelect("detail", "Niveau de dÃ©tail", ["brief", "balanced", "detailed"].map((value) => ({ value, label: brainPreferenceLabel("detailOption", value) })), brainPreferences.detail);
  ...
  const brainLanguageSelect = preferenceSelect("language", "Langue de rÃ©ponse", [{ value: "auto", label: "Langue de l'interface" }, { value: "fr", label: "Francais" }, { value: "en", label: "English" }, { value: "es", label: "Espanol" }, { value: "de", label: "Deutsch" }], brainPreferences.language);
  ...
  const brainPrivacySelect = preferenceSelect("provider.privacy", "Niveau de confidentialitÃ©", [{ value: "minimal", label: "Contexte minimal" }, { value: "full-context", label: "Contexte autorise complet" }], brainPreferences.provider.privacy);
  ...
  const customColorInput = element("input", {
    ...
    attributes: { type: "color", value: customAccentInitial, "aria-label": "Choisir une couleur d'accent personnalisÃ©e" }
  });
  const customColorSwatch = element("label", {
    className: `v8-accent-swatch v8-accent-swatch--custom${state.accent === "custom" ? " is-active" : ""}`,
    attributes: { "aria-label": "Accent personnalisÃ©", "data-tooltip": "Couleur personnalisÃ©e" }
  ...
  const soundPackControl = element("div", { className: "v8-sound-pack-control" }, [
    element("div", { className: "v8-sound-pack-control__actions" }, [
      ...
      actionButton({ actionId: "v8.sound.export", className: "v8-button v8-button--secondary", ariaLabel: "TÃ©lÃ©charger les sons en WAV", disabled: !soundSupported }, [icon("download"), element("span", { text: "TÃ©lÃ©charger (.WAV)" })])
    ]),
  ...
  }, [icon("scan-search"), element("span", { text: externalServices ? "VÃ©rifier le Worker" : "Worker indisponible" })]);
  ...
  const bannerFileInput = element("input", { attributes: { type: "file", accept: "image/png,image/jpeg,image/webp", "aria-label": "Choisir une banniÃ¨re", disabled: !canUploadMedia || null }, dataset: { profileMediaInput: "banner" } });
  const avatarMediaStatus = element("p", { className: "v8-settings-diagnostic-note", text: canUploadMedia ? "PNG, JPEG ou WebP, 5 Mo maximum." : "Disponible une fois connectÃ©.", attributes: { "aria-live": "polite" } });
  const bannerMediaStatus = element("p", { className: "v8-settings-diagnostic-note", text: canUploadMedia ? "Format large recommande, 5 Mo maximum." : "Disponible une fois connectÃ©.", attributes: { "aria-live": "polite" } });
  ...
  const page = element("section", { className: "v8-page v8-settings-page", dataset: { page: "settings" } }, [
    element("header", { className: "v8-page-heading" }, [
      element("div", { className: "v8-page-heading__copy" }, [
        element("span", { className: "v8-eyebrow", text: "SystÃ¨me" }),
        element("h1", { text: "RÃ©glages" }),
    ...
    element("div", { className: "v8-settings-layout" }, [
      element("aside", { className: "v8-settings-nav", attributes: { role: "tablist", "aria-orientation": "vertical", "aria-label": "Sections des rÃ©glages" } }, [
        ...
        element("button", { text: "SystÃ¨me", attributes: { type: "button", role: "tab", "aria-selected": "false", "aria-controls": "v8-settings-system", tabindex: "-1" }, dataset: { settingsSection: "v8-settings-system" } }),
      ...
      element("div", { className: "v8-settings-content" }, [
        element("section", { id: "v8-settings-profile", className: "v8-settings-section v8-surface", attributes: { role: "tabpanel", tabindex: "0" } }, [
          element("header", {}, [element("span", { className: "v8-eyebrow", text: "Identite" }), element("h2", { text: "Profil" }), element("p", { text: "Votre nom, votre photo et votre banniÃ¨re, visibles dans tout ETHONE." })]),
          ...
          element("div", { className: "v8-profile-media-row" }, [
            bannerPreviewHost,
            element("div", { className: "v8-profile-media-row__controls" }, [
              element("strong", { text: "BanniÃ¨re" }),
              bannerFileInput,
              bannerMediaStatus
            ])
          ]),
          settingRow("badge-check", "Notes de version (Changelog)", "Consulter l'historique des mises Ã  jour, nouveautÃ©s et correctifs d'ETHONE v194.", actionButton({ actionId: "v8.changelog.open", variant: "secondary" }, [icon("badge-check"), element("span", { text: "Ouvrir le Changelog" })]))
        ]),
        element("section", { id: "v8-settings-appearance", className: "v8-settings-section v8-surface", attributes: { role: "tabpanel", tabindex: "0", hidden: true } }, [
          element("header", {}, [element("span", { className: "v8-eyebrow", text: "Design System" }), element("h2", { text: "Apparence" }), element("p", { text: "Des rÃ©glages sobres, coherents et persistants." })]),
          element("div", { className: "v8-density-settings" }, [
            element("div", { className: "v8-density-settings__heading" }, [element("span", { className: "v8-setting-row__icon" }, [icon("sun-moon")]), element("div", {}, [element("strong", { text: "ThÃ¨me" }), element("p", { text: "Adapter les surfaces et le contraste." })]), themeResolved]),
            themeChoices
          ]),
          element("div", { className: "v8-density-settings" }, [
            element("div", { className: "v8-density-settings__heading" }, [element("span", { className: "v8-setting-row__icon" }, [icon("sparkles")]), element("div", {}, [element("strong", { text: "Ambiance Lumineuse (Aura)" }), element("p", { text: "Choisir l'atmosphÃ¨re colorÃ©e et les rÃ©flexions vitrÃ©es du Dashboard." })])]),
            auraChoices
          ]),
          element("div", { className: "v8-density-settings" }, [
            element("div", { className: "v8-density-settings__heading" }, [element("span", { className: "v8-setting-row__icon" }, [icon("type")]), element("div", {}, [element("strong", { text: "Typographie d'interface" }), element("p", { text: "Personnaliser le caractÃ¨re et la personnalitÃ© du texte." })])]),
            fontChoices
          ]),
          element("div", { className: "v8-density-settings" }, [
            element("div", { className: "v8-density-settings__heading" }, [element("span", { className: "v8-setting-row__icon" }, [icon("circle")]), element("div", {}, [element("strong", { text: "Courbure du Design" }), element("p", { text: "Ajuster l'Ã©chelle d'arrondi des cartes et boutons du systÃ¨me." })])]),
            radiusChoices
          ]),
          settingRow("palette", "Accent", "Identifier le Space et les actions importantes.", accentControls),
          element("div", { className: "v8-density-settings" }, [
            element("div", { className: "v8-density-settings__heading" }, [element("span", { className: "v8-setting-row__icon" }, [icon("rows-3")]), element("div", {}, [element("strong", { text: "Density Engine" }), element("p", { text: "Une densitÃ© coherente pour chaque page, panneau, widget et resolution." })]), densityResolved]),
            densityChoices,
            densityCustomHost,
          ...
          settingRow("layout-bottom", "Taille du Dock", "Changer l'Ã©chelle de la barre de navigation infÃ©rieure.", element("div", { className: "v8-dock-scale-options", attributes: { role: "group", "aria-label": "Taille du Dock" } }, [
          ...
          settingRow("align-center", "Alignement du Dock", "Dock centrÃ© flottant ou Ã©tendu sur la largeur.", element("div", { className: "v8-dock-scale-options", attributes: { role: "group", "aria-label": "Alignement du Dock" } }, [
            choice("v8.dock.align.center", "align-center", "CentrÃ©", !state.dockAlign || state.dockAlign === "center"),
            choice("v8.dock.align.stretch", "maximize-2", "Plein Ã‰cran", state.dockAlign === "stretch")
          ])),
          settingRow("sparkles", "Style du Dock (Verre)", "Transparence, flou artistique ou style opaque sobre.", element("div", { className: "v8-dock-scale-options", attributes: { role: "group", "aria-label": "Style du Dock" } }, [
            choice("v8.dock.glass.default", "sparkles", "VitrifiÃ©", !state.dockGlass || state.dockGlass === "default"),
          ...
          settingRow("eye-off", "Masquage auto du Dock", "Masquer automatiquement le dock et le rÃ©vÃ©ler au survol.", element("div", { className: "v8-dock-scale-options", attributes: { role: "group", "aria-label": "Masquage auto du Dock" } }, [
          ...
          settingRow("image", "BanniÃ¨re d'Accueil", "Afficher la salutation complÃ¨te, en version compacte ou la masquer.", element("div", { className: "v8-dock-scale-options", attributes: { role: "group", "aria-label": "BanniÃ¨re d'Accueil" } }, [
            choice("v8.home.hero.full", "image", "ComplÃ¨te", !state.homeHero || state.homeHero === "full"),
            choice("v8.home.hero.compact", "minimize-2", "Compacte", state.homeHero === "compact"),
            choice("v8.home.hero.hidden", "eye-off", "MasquÃ©e", state.homeHero === "hidden")
          ])),
          ...
          element("div", { className: "v8-density-settings" }, [
            ...
            settingRow("sparkle", "Effets d'ambiance", "Halos et particules animes en arriere-plan. Les desactiver allÃ©gÃ© le rendu en continu.", switchControl("v8.motion.ambient.toggle", "Activer les effets d'ambiance", state.ambientEffectsEnabled !== false)),
            settingRow("blend", "Flou d'interface", "Effet de verre depoli sur le dock, les fenÃªtrÃ¨s et menus. Le desactiver amÃ©liorÃ© la fluidite du defilement.", switchControl("v8.motion.blur.toggle", "Activer le flou d'interface", state.interfaceBlurEnabled !== false))
          ])
        ]),
        element("section", { id: "v8-settings-brain", className: "v8-settings-section v8-surface", attributes: { role: "tabpanel", tabindex: "0", hidden: true } }, [
          element("header", {}, [element("span", { className: "v8-eyebrow", text: "Personal Brain" }), element("h2", { text: "Assistant, memoire et confidentialitÃ©" }), element("p", { text: "Un contexte minimal, des permissions explicites et aucune clÃ© privÃ©e dans le navigateur." })]),
          settingRow("signature", "Identite", "Choisir le nom et le style de rÃ©ponse.", element("div", { className: "v8-brain-settings-controls" }, [brainPreferenceSwitch("enabled", "Activer Brain", brainPreferences.enabled), brainNameInput, brainPersonaSelect, brainDetailSelect])),
          ...
          settingRow("cpu", "Provider", "Les providers cloud exigent le backend ETHONE sÃ©curisÃ©.", element("div", { className: "v8-brain-settings-controls" }, [brainProviderSelect, brainModelInput, brainFallbackSelect, brainPrivacySelect])),
          settingRow("database", "Memoire", "PrÃ©fÃ©rences utiles uniquement, avec retention et RLS.", element("div", { className: "v8-brain-settings-inline" }, [brainPreferenceSwitch("memory.enabled", "Activer la memoire Brain", brainPreferences.memory.enabled), brainRetentionSelect, brainMemoryLoad])),
          settingRow("sunrise", "Briefing quotidien", "Ã‰vÃ©nements, prioritÃ©s et signaux utiles, en format concis.", element("div", { className: "v8-brain-settings-inline" }, [brainPreferenceSwitch("briefing.enabled", "Activer le briefing Brain", brainPreferences.briefing.enabled), brainPreferenceSwitch("briefing.concise", "Conserver un briefing concis", brainPreferences.briefing.concise)])),
          element("div", { className: "v8-brain-settings-block" }, [
            element("header", {}, [
              element("div", {}, [element("strong", { text: "Privacy Center" }), element("p", { text: "Chaque catÃ©gorie peut Ãªtre desactivee independamment." })]),
        ...
        element("section", { id: "v8-settings-sounds", className: "v8-settings-section v8-surface", attributes: { role: "tabpanel", tabindex: "0", hidden: true } }, [
          ...
          settingRow("volume-2", "Retours sonores", soundSupported ? "Activer ou couper tout le systÃ¨me audio." : "Le son n'est pas disponible dans ce navigateur.", switchControl("v8.sound.toggle", "Activer les sons", initialSoundPreferences.enabled, !soundSupported)),
          ...
          settingRow("audio-waveform", "Audio spatial", spatialSupported ? "Orienter trÃ¨s legerement les sons selon leur origine." : "L'audio spatial n'est pas disponible dans ce navigateur.", switchControl("v8.sound.spatial", "Activer l'audio spatial", initialSoundPreferences.spatial, !spatialSupported)),
        ...
        element("section", { id: "v8-settings-system", className: "v8-settings-section v8-surface", attributes: { role: "tabpanel", tabindex: "0", hidden: true } }, [
          element("header", {}, [element("span", { className: "v8-eyebrow", text: "Ã‰tat" }), element("h2", { text: "SystÃ¨me cloud & version" })]),
          element("div", { className: "v8-system-checks" }, [
            element("span", {}, [icon("shield-check"), element("strong", { text: "DonnÃ©es" }), element("b", { text: "RLS actif" })]),
  ...
  function renderWorkerStatus() {
    ...
    const rate = requests ? `${Math.round((successes / requests) * 100)} %` : "Non mesurÃ©";
    ...
    workerStatusHost.replaceChildren(
      workerMetric(snapshot.workerConnected ? "shield-check" : "shield-alert", "Worker", snapshot.workerConnected ? "ConnectÃ©" : "Non vÃ©rifiÃ©"),
      workerMetric("timer", "Latence", Number.isFinite(Number(snapshot.lastLatencyMs)) ? `${Math.round(snapshot.lastLatencyMs)} ms` : "Non mesurÃ©e"),
      workerMetric("chart-no-axes-combined", "SuccÃ¨s", rate),
      workerMetric("blocks", "Services", available.length ? available.join(", ") : "Aucun confirme"),
      workerMetric("gauge", "Rate limit", remaining),
      workerMetric("database", "Cache", Number.isFinite(Number(snapshot.cache?.entries)) ? `${snapshot.cache.entries} entree${snapshot.cache.entries === 1 ? "" : "s"}` : "Non mesurÃ©")
    );
  ...
  function updatePreferenceControls(nextState) {
    ...
    page.querySelectorAll("[data-theme-mode]").forEach((button) => {
      ...
      if (button.dataset.themeMode === "auto") {
        const note = button.querySelector(".v8-thÃ¨me-choice__resolved");
        if (note) {
  ...
  async function renderSettingsMemories() {
    ...
    brainMemoryStatus.textContent = "Chargement sÃ©curisÃ© depuis Supabase...";
    brainMemoryList.replaceChildren(statusState("loading", {
      title: "Chargement des memoires",
      description: "Lecture sÃ©curisÃ©e depuis Supabase.",
      compact: true,
      inline: true
    }));
    refreshIcons();
    ...
    brainMemoryList.replaceChildren(...(response.ok && response.data.length ? response.data.map((entry) => element("div", { className: "v8-settings-memory-row" }, [element("span", {}, [icon("bookmark")]), element("div", {}, [element("small", { text: entry.category }), element("strong", { text: entry.key }), element("p", { text: entry.value })]), element("div", {}, [element("button", { className: "v8-icon-button", attributes: { type: "button", "aria-label": "Modifier cette memoire" }, dataset: { settingsMemoryEdit: entry.id, settingsMemoryValue: entry.value } }, [icon("pencil")]), element("button", { className: "v8-icon-button", attributes: { type: "button", "aria-label": "Supprimer cette memoire" }, dataset: { settingsMemoryDelete: entry.id } }, [icon("trash-2")])])])) : [statusState(response.ok ? "empty" : "error", {
      ...
      description: response.ok ? "Les prÃ©fÃ©rences ajoutees volontairement apparaitront ici." : response.message,
      actions: response.ok ? [] : [element("button", { className: "v8-button v8-button--secondary", attributes: { type: "button" }, events: { click: () => void renderSettingsMemories() } }, [icon("refresh-cw"), element("span", { text: "RÃ©essayer" })])],
      compact: true,
      inline: true
    })]));
    refreshIcons();
  ...
  workerDiagnosticButton.addEventListener("click", async () => {
    ...
    workerDiagnosticButton.querySelector("span").textContent = "VÃ©rification";
    try {
    ...
    if (!controller.signal.aborted) {
      ...
      workerDiagnosticButton.querySelector("span").textContent = "VÃ©rifier le Worker";
      renderWorkerStatus();
    }
  ...
  async function handleProfileMediaChange(kind, input, statusEl) {
    ...
    try {
      ...
      statusEl.textContent = kind === "banner" ? "BanniÃ¨re mise a jour." : "Photo mise a jour.";
    } finally {
  ...
  function saveProfileName() {
    ...
    if (!nextName) {
      nameStatus.textContent = "Le nom ne peut pas Ãªtre vide.";
      return;
    }
