...
async function ownKey(env, auth) {
  ...
  const credential = await getUserProviderCredential(env, auth.userId, "henrik");
...
export async function henrikRoute({ env, url, route, auth }) {
  ...
  const result = await cachedLoad(`henrik:account:${name.toLowerCase()}:${tag.toLowerCase()}`, 120, async () => getValorantAccount(env, name, tag, await ownKey(env, auth)));
  return routeResult(result.data, { source: "henrik", cached: result.cached });
}
