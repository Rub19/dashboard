...
export async function lastFmRoute({ env, url, route }) {
  ...
  const loader = route.action === "recent-tracks"
    ? () => getRecentTracks(env, username, limit)
    : route.action === "top-artists"
      ? () => getTopArtists(env, username, period, limit)
      : () => getTopTracks(env, username, period, limit);
