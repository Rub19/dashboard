...
import { disconnectNotion, exchangeNotionCode, getRecentPages } from "../services/notion-oauth-client.js";
...
export async function notionPagesRoute({ env, auth }) {
  if (!auth?.userId) throw httpError("AUTH_REQUIRED", 401);
  const pages = await getRecentPages(env, auth.userId);
  return { data: { pages } };
}

export async function notionOAuthDisconnectRoute({ env, auth }) {
  if (!auth?.userId) throw httpError("AUTH_REQUIRED", 401);
  await disconnectNotion(env, auth.userId);
