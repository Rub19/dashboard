...
const ROUTE_CASES = Object.freeze([
  ...
  ["Tracker LoL matches", "/api/tracker/lol-matches?name=Player&tag=EUW&mode=all", (data) => data.length === 1 && data[0].metadata.agentName === "Ahri" && data[0].scoreboard.players[0].isMe === true && data[0].metadata.agentImageUrl.includes("/16.15.1/img/champion/Ahri.png") && data[0].scoreboard.players[0].items.length === 7 && data[0].scoreboard.players[0].items[0].image.includes("/16.15.1/img/item/1001.png") && data[0].scoreboard.players[0].items[0].name === "Bottes de vitesse" && data[0].scoreboard.players[0].assets.spells.length === 2 && data[0].scoreboard.players[0].assets.spells[0].name === "Flash"],
