...
const ROUTE_CASES = Object.freeze([
  ...
  ["Tracker", "/api/stats/apex-profile?platform=origin&identifier=Player", (data) => data.platform === "origin"],
  ["Tracker Valorant", "/api/stats/valorant-profile?name=Player&tag=EUW", (data) => data.handle === "Player#EUW"],
  ["Tracker Valorant matches", "/api/stats/valorant-matches?name=Player&tag=EUW&mode=all", (data) => data[0].metadata.score.team === 13 && data[0].metadata.score.opponent === 11 && data[0].metadata.score.roundsPlayed === 24 && data[0].scoreboard.players[0].isMe === true && data[0].scoreboard.players[0].assets.agent.small === "https://media.valorant-api.com/agents/e370fa57-4757-3604-3648-499e1f642d3f/displayicon.png"],
  ["Tracker LoL", "/api/stats/lol-profile?name=Player&tag=EUW", (data) => data.handle === "Player#EUW"],
  ["Tracker LoL matches", "/api/stats/lol-matches?name=Player&tag=EUW&mode=all", (data) => data.length === 1 && data[0].metadata.agentName === "Ahri" && data[0].scoreboard.players[0].isMe === true && data[0].metadata.agentImageUrl.includes("/16.15.1/img/champion/Ahri.png") && data[0].scoreboard.players[0].items.length === 7 && data[0].scoreboard.players[0].items[0].image.includes("/16.15.1/img/item/1001.png") && data[0].scoreboard.players[0].items[0].name === "Bottes de vitesse" && data[0].scoreboard.players[0].assets.spells.length === 2 && data[0].scoreboard.players[0].assets.spells[0].name === "Flash" && data[0].scoreboard.players[0].assets.rune.name === "Electrocute"],
  ["Tracker LoL ranked matches", "/api/stats/lol-matches?name=Player&tag=EUW&mode=ranked", (data) => data.length === 2 && data.every((m) => ["Ranked Solo", "Ranked Flex"].includes(m.metadata.modeName))],
  ["Tracker LoL normal matches", "/api/stats/lol-matches?name=Player&tag=EUW&mode=normal", (data) => data.length === 2 && data.every((m) => ["Normal Draft Pick", "Normal Blind Pick"].includes(m.metadata.modeName))],
  ["Tracker LoL ARAM matches", "/api/stats/lol-matches?name=Player&tag=EUW&mode=aram", (data) => data.length === 1 && data[0].metadata.modeName === "ARAM"],
